...
%          - lTol: Double defining differnece between single-slopes
        ...
        for ss2Ind=1:size(armt.SSprs, 3)
            ...
            if length(unique([s11Ind, s21Ind, s12Ind, s22Ind]))==2 &&...
                ...
                if sum(size(sdAll)==size(unique(sdAll, 'rows')))==2
                    ...
                    %Ordered slope 1 sources
                    s1Inds=s1Inds(ord1);
                    s2Inds=[s21Ind, s22Ind];
                    %Ordered slope 2 sources
                    s2Inds=s2Inds(ord2); 

                    d1Inds=[d11Ind, d12Ind];
                    %Ordered slope 1 detectors
                    d1Inds=d1Inds(ord1); 
                    d2Inds=[d21Ind, d22Ind];
                    %Ordered slope 2 detectors
                    d2Inds=d2Inds(ord2); 
